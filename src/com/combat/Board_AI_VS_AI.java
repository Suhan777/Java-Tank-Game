...
public class Board_AI_VS_AI extends JPanel implements ActionListener, KeyListener {
	//Global variables initialised at the start, and used later
	private Timer timer;
	private Timer timer2;
	public Hero_AI computer;
	public Hero_AI computer2;
	Image image;
	...
	private void init() {
		...
		computer = new Hero_AI(900, 350, 180, modeSelect, wallArray, powerupArray);
		...
		computer2 = new Hero_AI(50, 350, 0, modeSelect, wallArray, powerupArray);
		computer2.spawnGenerator();
		while (getValidRespawnPosition((int) computer2.getX(), (int) computer2.getY(), computer2.getImageWidth(),
				computer2.getImageHeight())) {
			computer2.spawnGenerator();
		}
	...
	private void checkBulletWallCollision() {
		ArrayList<Missile> heroMissiles = computer.getMissiles();
		ArrayList<Missile> hero2Missile = computer2.getMissiles();
	...
	private void checkCollision(Graphics g) {
		...
		Rectangle heroRectangle = computer.getBounds();
		Rectangle hero2Rectangle = computer2.getBounds();
		ArrayList<Missile> heroMissiles = computer.getMissiles();
		ArrayList<Missile> hero2Missile = computer2.getMissiles();
		...
		g2d.setTransform(at);
		at.rotate(Math.toRadians(computer2.getAngle()), computer2.getX() + computer2.getImageWidth() / 2,
				computer2.getY() + computer2.getImageHeight() / 2);
		...
		if (!hero2Area.isEmpty()) {
			play("tank-tank.wav");
			posxt2 = computer2.getX();
			posyt2 = computer2.getY();
			posyt1 = computer.getY();
			posxt1 = computer.getX();

			explodet2 = true;
			explodet1 = true;
			computer2.setCounter(0);
			computer2.spawnGenerator();
			while (getValidRespawnPosition((int) computer2.getX(), (int) computer2.getY(), computer2.getImageWidth(),
					computer2.getImageHeight())) {
				computer2.spawnGenerator();
		...
		for (int i = 0; i < heroMissiles.size(); i++) {
			...
			if (!hero2Area.isEmpty()) {
				...
				explodet2 = true;
				if (computer2.getCurrentPowerup() == 5) {
				} else {
					computer2.setCounter(0);
					p1score++;
					computer2.spawnGenerator();
					while (getValidRespawnPosition((int) computer2.getX(), (int) computer2.getY(),
							computer2.getImageWidth(), computer2.getImageHeight())) {
						computer2.spawnGenerator();
					}
				}
				heroMissiles.remove(i);
				computer2.setCurrentPowerup(0);
			}
		}
		...
		for (int i = 0; i < heroMissiles.size(); i++) {
			...
			if (!heroArea.isEmpty() && m1.isReflected()) {
				play("bullet-tank.wav");
				posxt1 = computer.getX(); // where collides
				posyt1 = computer.getY();
		...
		for (int i = 0; i < powerupArray.size(); i++) {
			...
			if (!hero2Area.isEmpty()) {
				play("powerup.wav");
				if ((powerupArray.get(i)).getPowerupType() == 5) {
					computer2.setIndex(1);
				}
				computer2.setIndex(1);
				int powerupType = (powerupArray.get(i)).getPowerupType();
				computer2.setCurrentPowerup(powerupType);
				powerupArray.remove(i);
			}
		}
		...
		for (Wall w : wallArray) {
			...
			if (!heroArea.isEmpty()) {
				computer.setX(computer.getPrevX());
				computer.setY(computer.getPrevY());
			}
		}
	}
	...
	private void drawGameOver(Graphics g) {
		...
		if (p1score > p2score) {
			...
			msg = "Congratulations COMPUTER 2" + "!";
			g.setColor(Color.white);
			g.setFont(small);
			g.drawString(msg, (1024 - fm.stringWidth(msg)) / 2, 768 / 2);
		}
	...
	private void drawPowerupAI2(Graphics g) {

		switch (computer2.getCurrentPowerup()) {
		case 0:
	...
	private void drawObjects(Graphics g) {
		...
		if (!pause) {
			...
			// hero2
			g2d.rotate(Math.toRadians(computer2.getAngle()), computer2.getX() + computer2.getImageWidth() / 2,
					computer2.getY() + computer2.getImageHeight() / 2);
			g2d.drawImage(computer2.getImage(), (int) computer2.getX(), (int) computer2.getY(),
					computer2.getImageWidth(), computer2.getImageHeight(), this);
			g2d.setTransform(old);

			// hero1 bullets
			ArrayList<Missile> ms = computer.getMissiles();
			for (Missile m : ms) {
				g2d.drawImage(m.getImage(), (int) m.getX(), (int) m.getY(), this);
			}

			// hero2 bullets
			ArrayList<Missile> ms2 = computer2.getMissiles();
			for (Missile m2 : ms2) {
	...
	public void update() {
		...
		if (start) {
			repaint();
			computer.setDestX(computer2.getX());
			computer.setDestY(computer2.getY());
			computer.play();

			computer2.setDestX(computer.getX());
			computer2.setDestY(computer.getY());
			computer2.play();
